...
def load_ifr(infection_fatality_root: Path) -> pd.DataFrame:
    data_path = infection_fatality_root / '20210118_v57_allage_ifr_by_loctime_v16_predbyranef_covidlocs.csv'
...
def load_ifr_risk_adjustment(infection_fatality_root: Path) -> pd.Series:
    data_path = infection_fatality_root / '20210118_v57_allage_ifr_by_loctime_v16_predbyranef_covidlocs_agegtlt65.csv'
    data = pd.read_csv(data_path)
...
def load_ifr_data(infection_fatality_root: Path) -> pd.DataFrame:
    data_path = infection_fatality_root / 'dev_output_dirs' / '57_rsoren' / 'df_prepped_ifr_v16.csv'
    data = pd.read_csv(data_path)
...
def load_ihr(infection_hospitalization_root: Path) -> pd.DataFrame:
    data_path = infection_hospitalization_root / '20210118_v57_allage_ihr_by_loctime_v16.csv'
    data = pd.read_csv(data_path)
    data['date'] = pd.to_datetime(data['datevar'])
...
def load_ihr_data(infection_hospitalization_root: Path) -> pd.DataFrame:
    data_path = infection_hospitalization_root / 'dev_output_dirs' / '57_rsoren' / 'df_prepped_ihr_v16.csv'
    data = pd.read_csv(data_path)
    data['date'] = pd.to_datetime(data['date'])
