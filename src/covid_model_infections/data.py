...
def load_model_inputs(model_inputs_root:Path, hierarchy: pd.DataFrame, input_measure: str,
                      excess_mortality: bool = True,
                      fh_subnationals: bool = False, gbd: bool = False,) -> Tuple[pd.Series, pd.Series, Dict]:
    if fh_subnationals:
...
def load_hierarchy(model_inputs_root:Path, fh_subnationals: bool, gbd: bool,) -> pd.DataFrame:
    if fh_subnationals:
    ...
    if fh_subnationals:
        ...
        for location_id, parent_id, parent_name in [(3543, 60886, 'King and Snohomish Counties'),
                                                    (3563, 60887, 'Washington except for King, Snohomish, and Spokane Counties'),
                                                    (3530, 60887, 'Washington except for King, Snohomish, and Spokane Counties'),]:
